    ...
    function updateAlbumTracks($data = NULL, $albumId= NULL)
    {
        ...
        $query = DB::select("UPDATE `tracks`
                                    SET `artist` = ?,
                                        ...
                                        `subGenreId` =  ?
                                    WHERE `albumid` = ?",
                                    [urlencode($artist), urlencode($album), urlencode($time), urlencode($company), urlencode($website), urlencode($moreInfo), urlencode($producers), urlencode($writer), $admin_id, $availableMembers, $reviewable, $logos, $client, $whiteLabel, $video, $embedlink, $graphics, $albumType, $website1, $website2, $bpm, $facebookLink, $twitterLink, $instagramLink, $genre, $subGenreId, $albumId]);
   ...
   function getMembersList()
	...
	function getGenres_album()
    {
     ...
     function updateTrack_trm($data, $trackId)
     {
         ...
         $query = DB::select("update `tracks` set artist = ?, title = ?, featured_artist_1 = ?, featured_artist_2 = ?, album = ?, time = ?, label = ?, link = ?,
         moreinfo = ?,
         notes = ?,
         producer = ?, writer = ?, edited = NOW(), editedby = ?, active = ?, review = ?, logos = ?, whitelabel = ?,  videoURL = ?, embedvideoURL = ?, graphicscomplete =  ?, albumType =  ?, link1 =  ?, link2 =  ?, bpm =  ?, facebookLink =  ?, twitterLink =  ?, instagramLink =  ?, tiktokLink =  ?, snapchatLink =  ?, othersLink =  ?, genreId =  ?, status = ?, memberPreviewAvailable = ?, contact_name = ?, contact_email = ?, contact_phone = ?, relationship_to_artist = ?,feedback1_contact_email = ?, feedback2_contact_email = ?, feedback3_contact_email = ?, songkey = ?, client = ? where id = ?", [addslashes(urlencode($artist)), addslashes(urlencode($title)), urlencode($featured_artist_1), urlencode($featured_artist_2), urlencode($album), urlencode($time), urlencode($company), urlencode($website), urlencode($moreInfo), urlencode($notes), urlencode($producers), urlencode($writer), $admin_id, $availableMembers, $reviewable, $logos, $whiteLabel, $video, $embedlink, $graphics, $albumType, $website1, $website2, $bpm, $facebookLink, $twitterLink, $instagramLink, $tiktokLink, $snapchatLink, $othersLink, $genre, $status, $memberPreviewAvailable, addslashes($contact_name), $contact_email, $contact_phone, $relationship_to_artist, $second_contact_email, $third_contact_email, $fourth_contact_email, $songkey, $client, $trackId]);
     ...
     function addTrackupdate_trm($data, $track_id)
     {
       ...
       /*  $query = DB::select("UPDATE `tracks` SET artist = '" . $artist . "', featured_artist_1 = '" .  $featured_artist_1 . "', featured_artist_2 = '" .  $featured_artist_2 . "', title = '" .  $title . "', albumType = '" . $albumType . "', album = '" . $album . "', time = '" . $time . "', label = '" . $company . "', moreinfo = '" . $moreInfo . "', producer = '" . $producers . "', writer = '" . $writer . "', addedby = '" . $_COOKIE['adminId'] . "', active = '" . $availableMembers . "', review = '" . $reviewable . "', client = '" . $client . "', whitelabel = '" . $whiteLabel . "', type = '" . $type . "', graphicscomplete = '" . $graphics . "', genreId = '" . $genre . "', subGenreId = '" . $subGenre . "', bpm = '" . $bpm . "', status = '" . $status . "'  WHERE id = '" . $track_id . "'"); */
          ...
          $query = DB::select('UPDATE `tracks` SET artist = ?, featured_artist_1 = ?, featured_artist_2 = ?, title = ?, albumType = ?, album = ?, time = ?, label = ?, moreinfo = ?, producer = ?, writer = ?, addedby = ?, active = ?, review = ?, client = ?, whitelabel = ?, type = ?, graphicscomplete = ?, genreId = ?, subGenreId = ?, bpm = ?, status = ?  WHERE id = ?', [$artist, $featured_artist_1, $featured_artist_2, $title, $albumType, $album, $time, $company, $moreInfo, $producers, $writer, $admin_id, $availableMembers, $reviewable, $client, $whiteLabel, $type, $graphics, $genre, $subGenre, $bpm, $status, $track_id]);      
         return $query;
...

{
 ...
 $result =  DB::select("update product_questions set question = ? where question_id = ?", [addslashes($question), $question_id]);
...

       {
         ...
         echo $option; echo '<br />';
...

          {
           ...
           DB::select("update product_answers set answer = ? where answer_id = ?", [$option, $answer_ids[$i]]);   
...
if(empty($merch_digi)){
 ...
 $query =  DB::select("update products set company_id = ?, name = ?, link = ?, moreinfo = ?, added = NOW(), addedby = ?,merch_status = ?, active = ?, review = ?, model = ?, product_details = ?,  product_technology = ?, product_gender = ?, division = ? where product_id = ?", [$company, $productName, $link, addslashes($moreInfo), $admin_id, $merch_digi, $active, $reviewable, $model, $productDetails, $benefits, $gender, $brand, $pid]);  
...
return $question_id;
  ...
  function adc_updateClient($data, $clientId)
  {
     ...
     if(isset($activate_track_review_feedback) && $activate_track_review_feedback == 1){
      ...
      $admin_id = Auth::user()->id;
      $query = DB::update("update `clients` set name = ?, editedby = ?, edited = NOW(), ccontact = ?, address1 = ?, address2 = ?, city = ?, state = ?, country = ?, website = ?, zip = ?, phone = ?, mobile = ?, trackReviewEmailsActivated = ? where id = ?", [urlencode($companyName), $admin_id, urlencode($name), urlencode($address1), urlencode($address2), urlencode($city), urlencode($state), urlencode($country), urlencode($website), $zip, urlencode($phone), urlencode($mobileNo), $trackReviewsActivate, $clientId]);
      return $query;
  }
  ...
  function adc_addClient($data)
  {
      ...
      if ($userRows1 < 1 && $userRows2 < 1 && $userRows3 < 1 && $userRows4 < 1) {
        ...
        DB::table('client_social_media')->insert($clientSocialMediaData);
 ...
 where memberId = '$memberId'");
...

  {
      ...
      $query = DB::select("select *  from  member_subscriptions	where member_Id = ? and status = '1' order by subscription_Id desc limit ?, ?", [$memberId, $start, $limit]);
      $query = DB::select("select *  from  member_subscriptions	where member_Id = ? and status = '1'", [$memberId]);
      $result['numRows']  = count($query);
...

  {
      ...
      $result = DB::update("update members set active = '-1'  where id = ?", [$memberId]);
...

  {
      ...
      $result = DB::delete("delete from members where id = ?", [$memberId]);
