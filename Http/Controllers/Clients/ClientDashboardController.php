...
class ClientDashboardController extends Controller
{
	...
	public function viewClient_dashboard(Request $request)
	{
		...
		$output = array();
        $output['pageTitle'] = 'Digiwaxx Client Dashboard';
        $output['packageId'] = 2;
	...
	public function viewClientEditProfile(Request $request){
		...
		$output = array();
        $output['pageTitle'] = 'Digiwaxx Client Info';
		...
		if(isset($_POST['updateClient'])){
				...
				if(isset($_FILES['profileImage']['name']) && strlen($_FILES['profileImage']['name'])>3){
				   ...
				   if (!file_exists("./client_images/".$clientId."/")){
					  ...
					  mkdir("./client_images/".$clientId, 0755, true);
	...
	public function viewClientChangePasssword(){
		...
		$output = array();
        $output['pageTitle'] = 'Digiwaxx Client Change Passsword';
	...
	public function viewClienDashboardRated(){
		...
		$output = array();
        $output['pageTitle'] = 'Digiwaxx Client Dashboard Rated';
...
/* 		foreach($countryCodes as $code) {
	...
	public function viewClienDashboardDownloaded(){
		...
		$output = array();
        $output['pageTitle'] = 'Digiwaxx Client Dashboard Downloaded';
...
/* 		foreach($countryCodes as $code) {
	...
	public function viewClienDashboardCommented(){
		...
		$output = array();
        $output['pageTitle'] = 'Digiwaxx Client Dashboard Commented';
...
/* 		foreach($countryCodes as $code) {
	...
	public function viewClienDashboardPlayed(){
		...
		$output = array();
        $output['pageTitle'] = 'Digiwaxx Client Dashboard Played';
...
/* 		foreach($countryCodes as $code) {
	...
	public function viewClientTrackReview(){
		...
		$getTrackInfoClient = $this->clientAllDB_model->getTrack($_GET['tId']);

		if(isset($getTrackInfoClient['data']) && count($getTrackInfoClient['data']) >0){
			$trackData = $getTrackInfoClient['data'];


			foreach($trackData as $trakDetObj){
				$sessCltId = Session::get('clientId');
				$trakCltId = $trakDetObj->client;

				if(empty($trakCltId) || ($trakCltId != $sessCltId)){
					$isValidClient = 0;
					break;
				}
			}
		...
		$output = array();
        $output['pageTitle'] = 'Digiwaxx Client Track Review';
...

  {
	...
	$output['comments'] = $this->clientAllDB_model->getTrackComments($_GET['tId'],$start,$limit); 
	$output['periodicdownloads'] = $this->clientAllDB_model->getPeriodicDownloads($_GET['tId']);
	    $output['packageId'] = 2;
        $output['displayDashboard'] = 1;
	...
	public function Client_tracks(){
		...
		$output = array();
        $output['pageTitle'] = 'Digiwaxx Client Tracks';
        $output['packageId'] = 2;
...
// -----------------------------------------------Pcloud Image Upload------------------------------------------------
	...
	public function Client_submit_track(Request $request){
		...
		$output = array();
        $output['pageTitle'] = 'Digiwaxx Client Submit Track';
        $output['packageId'] = 2;
	...
	function buildMultiPartRequest($ch, $boundary, $fields, $files) {
		...
		public function get_audio_bitrate( Request $request){
			...
			if (!file_exists($filename)) { 
	 		...
	 		// echo json_encode(["bitRate"=>320,"bpm" => 320,"key"=>'']);exit();
			...
			$fileData = array('bitRate' => 320, 'sampleRate' => 0);
			$fp = fopen($filename, 'r');
			if (!$fp) {
				return false;
			}
	  ...
	  // pagination ends
		...
		public function Client_label_reps(Request $request){
			...
			$output = array();
			 $output['pageTitle'] = 'Digiwaxx Client label Reps';
			$output['packageId'] = 2;
	  ...
	  // pagination ends
		...
		public function Client_add_label_reps(Request $request){
			...
			$output = array();
			 $output['pageTitle'] = 'Digiwaxx Client Add Label Reps';
			$output['packageId'] = 2;
		...
		public function Client_edit_label_reps(Request $request){
			...
			$output = array();
			 $output['pageTitle'] = 'Digiwaxx Client Edit Label Reps';
			$output['packageId'] = 2;
		...
		public function Client_messages(Request $request){
			...
			$output = array();
			 $output['pageTitle'] = 'Digiwaxx Client Messages';
			$output['packageId'] = 2;
	   ...
	   /*	if(($subscriptionInfo['numRows']>0) && ($subscriptionInfo['data'][0]->packageId==2))
		...
		public function Client_messages_unread(Request $request){
			...
			$output = array();
			 $output['pageTitle'] = 'Digiwaxx Client Unread Messages';
			$output['packageId'] = 2;
		...
		public function Client_messages_starred(Request $request){
			...
			$output = array();
			 $output['pageTitle'] = 'Digiwaxx Client Starred Messages';
			$output['packageId'] = 2;
			...
			$headerOutput['displayDashboard'] = 1;
			 ...
			 $headerOutput['pageTitle'] = 'Digiwaxx Client Messages';
...
if ($_GET['page'] > $numPages && $numPages > 0) {
	   ...
	   exit;
			...
			$output = array();
			 $output['pageTitle'] = 'Digiwaxx Client Messages Conversation';
			$output['packageId'] = 2;
			...
			$output = array();
			 $output['pageTitle'] = 'Digiwaxx Client Payment';
			$output['packageId'] = 2;
			...
			public function client_submit_track_preview(Request $request){
				...
				$output = array();
				 $output['pageTitle'] = 'Digiwaxx Client Submit Track Preview';
				$output['packageId'] = 2;
			...
			public function Client_submit_track_edit(Request $request){
				...
				$output = array();
				 $output['pageTitle'] = 'Digiwaxx Client Edit Track';
				$output['packageId'] = 2;
			...
			// 	}
				...
				$date_time_array = getdate(time());
					...
					if (isset($_POST['amrkeynum']) && is_array($_POST['amrkeynum'])) {
						$trackId = $_GET['tId'];
						
						foreach ($_POST['amrkeynum'] as $amrkeynum) {
							$track_num = (int) preg_replace('/\D/', '', $amrkeynum);
							$track_title_key = "title" . $track_num; 
							$track_title = isset($_POST[$track_title_key]) ? $_POST[$track_title_key] : "";

							if (!empty($_FILES[$amrkeynum]['name']) && $track_title) {
								$audio_single = $request->file($amrkeynum);
								$destination_path = base_path('AUDIO/');
								...
								$fileName = md5(rand(1000, 10000));
								$fileNameToStore = $clientId . '_' . $amrkeynum . '_' . $fileName . '.mp3';
								...
								if (!empty($uploaded_data)) {
									...
									$version = "";
									$metadata = $this->add_to_pcloud($destination_path, $fileNameToStore);
									$file_id = $metadata->metadata->fileid;
					
									$functionName = "addClientTrackAmr_cld" . $track_num;
									$this->clientAllDB_model->$functionName($trackId, $file_id, $clientId, $version, $title);
								}
							}
						}
					}
  ...
  // pagination
...

	{
		...
		$output['packageId'] = 2;
		$reviewCount = DB::table('tracks_reviews')
						->where('track', $_GET['tId'])
						->count();
		$output['reviewsCount'] = $reviewCount;
			...
			if($fileCount > 0){
				 ...
				 for ($i = 1; $i <= $fileCount; $i++) {
					 if (isset($_FILES['audio' . $i]['name']) && strlen($_FILES['audio' . $i]['name']) > 4) {
						 ...
						 $audioFileNameOrig = $audio_single->getClientOriginalName();
						 ##echo $audioFileNameOrig;die();
						 $audio_ext = $audio_single->getClientOriginalExtension();

						 //Display File Real Path
						 $audio_real_path = $audio_single->getRealPath();
						 // echo 'File Real Path: '.$real_path;
					
						
						 //Display File Size
						 $audio_file_size = $audio_single->getSize();						 

						 if($audio_ext != 'mp3'){
					 
							 header("location: " . url("Client_edit_track?tid=" . $_GET['tId'] . "&noaudio=1"));
							 exit;
							...
							$base_name = pathinfo($audioFileNameOrig, PATHINFO_FILENAME);
							$extension = pathinfo($audioFileNameOrig, PATHINFO_EXTENSION);
							$file_name = $_GET['tId'] . '_' . $base_name . '_' . $timestamp . '.' . $extension;
							 ...
							 if( !empty( $uploaded_data )){
								...
								$fileMetadata = $this->add_to_pcloud($destination_path, $file_name);

								##echo '<pre>';print_r($fileMetadata);
								...
								$this->clientAllDB_model->new_track_version_submit($_GET['tId'], $file_name, $version, $pcloudFileId);			
								$submitted_versions[] = $version;		

							 }
					 }
					
				 }			
				/* -- Email  to client code starts -- */
				if(!empty($submitted_versions)){
					$contact_email = $_POST['contact_email'];					
					$email_data = array(
						'artist' => $_POST['artist'],
						'contact_email' => $contact_email,
						'track_id' => $_GET['tId'], 
						'track_title' => $_POST['title'] ,
						'submitted_versions' => $submitted_versions,
						'client_name' => $client_name,
					);
					// send email to client
					try {
						Mail::send('mails.clients.versionSubmitted', ['email_data' => $email_data], function ($message) use ($email_data) {
							$message->to($email_data['contact_email']);
							$message->subject('Tracks Versions submitted successfully.');
							$message->from('business@digiwaxx.com', 'Digiwaxx');
						});

						// send email to admin
						Mail::send('mails.clients.adminEmailVersionSubmitted', ['email_data' => $email_data], function ($message) use ($email_data) {													
							$message->to('music@digiwaxx.com');
							$message->subject('New Tracks Versions submitted.');
							$message->from('business@digiwaxx.com', 'Digiwaxx');
						});

						$versionIds = DB::table('tracks_submitted_versions')
						->where('track_id', $email_data['track_id'])
						->where('client_email_sent', 0)
						->where('admin_email_sent', 0)
						->pluck('id')
						->toArray();
						...
						if (!empty($versionIds)) {
							DB::table('tracks_submitted_versions')
								...
								->update(['client_email_sent' => 1]);

							DB::table('tracks_submitted_versions')
							->whereIn('id', $versionIds)
							->update(['admin_email_sent' => 1]);
						}
	...
	/*
	 ...
	 $countVersion =  $_POST['numVersion'];
		...
		$output['track'] = $getTrackInfoClient;
		$output['track_submitted_versions'] = $this->clientAllDB_model->getTrackSubmittedVersions($_GET['tId']);
...
// 		die();
    ...
    public function client_my_package(Request $request){
		...
		$output = array();
        $output['pageTitle'] = 'Digiwaxx Client Subscription Details';
	...
	public function deleteClientSubmittedTrack(Request $request)
	{
		$fileId = $request->input('file_id');
		if (!$fileId) {
			return response()->json(['message' => 'Invalid Track ID'], 400);
		}
	
		$track = DB::table('tracks_submitted')
			->where(function ($query) use ($fileId) {
				foreach (['amr1', 'amr2', 'amr3', 'amr4', 'amr5', 'amr6'] as $column) {
					$query->orWhere($column, $fileId);
				}
			})->first();
		...
		$columnToUpdate = collect(['amr1', 'amr2', 'amr3', 'amr4', 'amr5', 'amr6'])
			->first(fn($column) => $track->$column == $fileId);
	
		if ($columnToUpdate) {
			DB::table('tracks_submitted')->where('id', $track->id)->update([$columnToUpdate => null]);
			return response()->json(['message' => 'Track deleted successfully'], 200);
		}
	
		return response()->json(['message' => 'Track deletion failed'], 500);
	}

	public function deleteClientTrack(Request $request)
	{
		\Log::info('Delete track request received', ['file_id' => $request->input('file_id')]);
		$fileId = $request->input('file_id');
		if (!$fileId) {
			return response()->json(['message' => 'Invalid Track ID'], 400);
		}
	
		$track = DB::table('tracks_mp3s')->where('location', $fileId)->first();
	
		if (!$track) {
			return response()->json(['message' => 'Track not found'], 404);
		}
	
		$update = DB::table('tracks_mp3s')->where('id', $track->id)->update(['location' => null]);
	...
	## List all of the Tracks for which there exist atleast 1 submitted track version 
	public function client_submitted_tracks_versions(Request $request){
        ...
        $output['pageTitle'] = 'Digiwaxx - Submitted Tracks Versions';
        $output['packageId'] = 2;
		...
		$output['currentPage'] = 'uploaded_tracks_versions';	

		$where = " WHERE tracks.client='".$clientId."' AND tracks_submitted_versions.approved = '0' AND tracks_submitted_versions.deleted = '0' AND tracks.deleted = '0'";
		 ...
		 $num_records = $this->clientAllDB_model->getNumSubmittedTracksVersions_trm($where);
		 $numPages = (int) ($num_records / $limit);
		 $reminder = ($num_records % $limit);
		 ...
		 // pagination
		 $result = $this->clientAllDB_model->getSubmittedTracksVersions_trm($where, $start, $limit);
		 $output['submitted_tracks'] = $result['data'];
		 $output['tracks'] = $this->clientAllDB_model->getClientFooterTracks($clientId);
		 $output['rightTracks'] = $this->clientAllDB_model->getRightTracks($clientId);

		 return view('clients.dashboard.Clientsubmitted_tracks_versions', $output);exit;

		//dd($output);	
	}

	## Client - list all of the submitted track versions(Client & Track Based) 

	public function view_uploaded_tracks_versions(Request $request){
		$output = array();
		$mainTrakId = '';
		if(empty(Session::get('clientId'))){
			return redirect()->intended('login');
		...
		if($request->has('tid') && !empty($request->tid)){
			$mainTrakId = $request->tid;
		}else{
			return redirect()->intended('uploaded_tracks_versions');
		}
		...
		$where = " WHERE tracks.client='".$clientId."' AND tracks_submitted_versions.approved = '0' AND tracks_submitted_versions.deleted = '0' AND tracks.deleted = '0' AND tracks.id = '" . $mainTrakId . "' ";

		//echo $where;die();
		
		$output['result'] = $this->clientAllDB_model->getSubmittedVersionsForTrack($where);
		 $output['tracks'] = $this->clientAllDB_model->getClientFooterTracks($clientId);
		 $output['rightTracks'] = $this->clientAllDB_model->getRightTracks($clientId);		
		return view('clients.dashboard.Client_view_uploaded_tracks_versions', $output);exit;		
	}	
