		...
		// }
			...
			if ($result > 0) {
				...
				if (!empty($client_email) && count($client_email) > 0 ) {
					...
					Mail::send('mails.members.reviewAdded', ['data' => $data], function ($message) use ($data) {
   ...
   function delete_pcloud($fileid)
    ...
    $memId=Session::get('memberId');
...

	  {
		...
		if (!file_exists("./member_images/".$memberId_from_session."/")) 

		{

			mkdir("./member_images/".$memberId_from_session);
	...
	public function Member_track_review_share(Request $request)
	{
	   ...
	   if(isset($_POST['additionalThingsReview']))
	   {
		   ...
		   if($result>0) {
		       ...
		       $emailofuser = $email;
		       $data = array('userEmailId' => $emailofuser, 'name' => $memName,'trackname' => $trackTitle,'trackArtist' => $artist,'additional_things_requested' => $request_additional_things,'clientEmailId' => $labelContactEmail);
