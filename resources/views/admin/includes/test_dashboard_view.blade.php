									...
									<div class="widget-main no-padding">
										<div id="reviews-container">
											...
											<div id="pagination-controls">
												@if ($dashboard_weekly_tracks_reviews->currentPage() > 1)
												<button id="load-previous-reviews" data-prev-page="{{ $dashboard_weekly_tracks_reviews->currentPage() - 1 }}" class="btn btn-secondary">
													Previous
												</button>
												@endif

												@if ($dashboard_weekly_tracks_reviews->hasMorePages())
												<button id="load-more-reviews" data-next-page="{{ $dashboard_weekly_tracks_reviews->currentPage() + 1 }}" class="btn btn-primary">
													Next
												</button>
												@endif
											</div>
...
<script>
	$(document).ready(function() {
		function loadReviews(page) {
			var reviewsUrl = "{{ route('weekly.reviews') }}";
			$('.processing_loader_gif').show();

			$.ajax({
				url: reviewsUrl + "?page=" + page,
				type: "GET",
				success: function(data) {
					$('.dialogs').html(data.reviewsHtml);
					if (data.hasMorePages) {
						$('#load-more-reviews').data('next-page', page + 1);
					...
					if (page > 1) {
						$('#load-previous-reviews').data('prev-page', page - 1);
						$('#load-previous-reviews').show();
					} else {
						$('#load-previous-reviews').hide();
					}
				},
		...
		$('#reviews-container').on('click', '#load-more-reviews', function(e) {
			e.preventDefault();
			var nextPage = $(this).data('next-page');
			loadReviews(nextPage);
		});

		$('#reviews-container').on('click', '#load-previous-reviews', function(e) {
			e.preventDefault();
			var prevPage = $(this).data('prev-page');
