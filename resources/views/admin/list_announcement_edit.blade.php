...
<div class="main-content">
    <div class="main-content-inner">
        ...
        <div class="breadcrumbs" id="breadcrumbs">
            ...
            <ul class="breadcrumb">
                <li class="active"><i class="ace-icon fa fa-list list-icon"></i> EDIT ANNOUNCEMENT</li>
            </ul>
        </div>
        <!-- /section:basics/content.breadcrumbs -->
        <?php
        // 		print_r($news_details);
        foreach ($announ_edit as $news_detail) {
            $n_title = $news_detail->ma_title;
            $n_desc = $news_detail->ma_description;
            $n_aval = $news_detail->ma_availability;
        }
        ?>
        <div class="page-content">
            <div class="row">
                <div class="col-xs-12">
                    <div class="news-form">
                        <?php $route = route('announcement_edit'); ?>
                        <form method="post" action="<?php echo $route; ?>" enctype=multipart/form-data>
                            @csrf
                            <input type="hidden" name="announ_id" value="<?php echo $announcement_id ?>" <div class="form-group">
                            <label for="news_title">TITLE</label>
                            <input type="text" class="form-control" id="edit_announ_title" name="edit_announ_title" aria-describedby="emailHelp" placeholder="Enter Title" value="<?php echo $n_title ?>">
                            ...
                            <div class="form-group">
                                ...
                                <div class="row">
                                    <div class="col-xs-12">
                                        ...
                                        @csrf
                                        <textarea class="ckeditor" name="edit_announ_description"><?php echo stripslashes(urldecode($n_desc)); ?></textarea>
                                        <br />
                            ...
                            <div class="form-group form-check">
                                <label class="form-check-label" for="edit_ann_avail">Availability</label>
                                <select class="form-select" aria-label="Default select example" name="edit_ann_avail" id="select_check" onchange="my_select()">
                                    <option value="All" <?php if ($n_aval == 'All') {
                                                            echo "selected";
                                                        } ?>>All</option>
                                    <option value="Selected_Members" <?php if ($n_aval != 'All') {
                                                                            echo "selected";
                                                                        } ?>>Selected Members</option>

                                </select>
                            </div>
                            <div class="dyn_avail" style="<?php if ($n_aval == 'All') {
                                                                echo 'display:none';
                                                            } else {
                                                                echo 'display:block';
                                                            } ?>">
                                ...
                                <div class="form-group form-check">
                                    ...
                                    <div class="selected_members">
                                        <?php if (!empty($mem_details)) { ?>
                                            ...
                                            ?>
                                                    <span id="member_id_<?php echo $value1->id;  ?>">
                                                        <input type="text" id="sel_item" name="selected_mem_id[]" value="<?php echo $value1->fname; ?>">
                                                        <button id="remove" class="btn btn-danger" type="button" onclick="remove_val('<?php echo $value1->fname ?>','<?php echo $value1->id ?>')">X</button>
                                                        <input type="hidden" id="hidden_sel_item" name="hidden_selected_mem_id[]" value="<?php echo $value1->id; ?>">
                                                    </span>
