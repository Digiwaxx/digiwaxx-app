@extends('layouts.client_track_dashboard')
...
<style>
    ...
    .fileinput-button.btn-drag button {

        background: rgb(82, 208, 248);

        background: -moz-linear-gradient(135deg, rgba(82, 208, 248, 1) 0%, rgba(219, 55, 143, 1) 100%);

        background: -webkit-linear-gradient(135deg, rgba(82, 208, 248, 1) 0%, rgba(219, 55, 143, 1) 100%);

        background: linear-gradient(135deg, rgba(82, 208, 248, 1) 0%, rgba(219, 55, 143, 1) 100%);

        filter: progid:DXImageTransform.Microsoft.gradient(startColorstr="#52d0f8", endColorstr="#db378f", GradientType=1);
    ...
    .fileinput-button.btn-drag button:hover {

        background: -moz-linear-gradient(135deg, rgba(219, 55, 143, 1) 0%, rgba(82, 208, 248, 1) 100%);

        background: -webkit-linear-gradient(135deg, rgba(219, 55, 143, 1) 0%, rgba(82, 208, 248, 1) 100%);

        background: linear-gradient(135deg, rgba(219, 55, 143, 1) 0%, rgba(82, 208, 248, 1) 100%);
...
@section('content')
   ...
   <section class="main-dash">
    ...
    <!--<h1>Add Track New </h1>-->
    <div class="main-content dash-container add-track-page">
        <div class="main-content-inner">
                                                ...
                                                <tr class="template-download">
                                                    <td>
                                                        <span class="preview">
                                                            @if (!empty($mp3->audio_url))
                                                                <audio controls>
                                                                    <source src="{{ $mp3->audio_url }}" type="audio/mpeg">
                                                    ...
                                                    <td class="left">
                                                        <input type="radio" value="{{ $mp3->id }}"
                                                            id="preview{{ $mp3->id }}" name="preview" {{ $mp3->preview == 1 ? 'checked' : '' }} >
                                                    </td>
                                                    <td>
                                                    ...
                                                    <td>
                                                        <p class="name">
                                                            ...
                                                            <a href="{{ $mp3->audio_url }}" title="{{ $mp3->title }}"
            ...
            {{--  --}}
            {{-- <table role="presentation" class="table table-striped">  <tbody class="files"><tr class="template-upload fade in"> </tr>--}}
            ...
            <script>
                ...
                window.lsData['urlSaveMp3'] = "{{ route('save.client.mp3.track' , ['track_id' =>  isset($track_id) && !empty($track_id) ? Crypt::encryptString($track_id) : ''] ) }}";
                window.lsData['upload_route'] = "{{ route('client_mp3_upload', ['track_id' =>  isset($track_id) && !empty($track_id) ? Crypt::encryptString($track_id) : '']) }}";
                window.lsData['step3'] = "{{ route('manage_client_track_meta_info', ['id' =>  isset($track_id) && !empty($track_id) ? Crypt::encryptString($track_id) : '']) }}";
...
<script src="https://digiwaxx.com/digiwaxx-dev/assets_admin/assets/js/bootstrap.js"></script>
            ...
            <script src="{{ asset('public/assets/js/add_client_audio_files.js') }}"></script>
